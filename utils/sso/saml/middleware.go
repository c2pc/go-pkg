...
func (m *Middleware) ServeHTTP(c *gin.Context) {
	if c.Request.URL.Path == m.ServiceProvider.MetadataURL.Path {
		m.ServeMetadata(c)
		return
	}
...
func (m *Middleware) ServeMetadata(c *gin.Context) {
	...
	c.Writer.Header().Set("Content-Type", "application/samlmetadata+xml")
...
func (m *Middleware) ServeACS(c *gin.Context) {
	...
	m.CreateSessionFromAssertion(c.Writer, c.Request, assertion, m.ServiceProvider.DefaultRedirectURI)
}
